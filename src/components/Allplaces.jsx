    ...
    (allPlacesArr.length === 0)? 
      ...
      (<div className='d-flex flex-wrap justify-content-center align-items-center m-auto' style={{width:"80vw"}}>
          {allPlacesArr?.map((items, index) => (
              <div className="card all-places-card m-3 position-relative" style={{ width: "18rem"}} key={index}>
                  {items?.photos?.[0]? 
                  (
                      <img
                  ...
                  <div className="card-body ">
                    <div className='d-flex justify-content-between'>
                      ...
                      <div>{items.rating ? items.rating : 0}⭐ <b>|</b> {items.user_ratings_total ? items.user_ratings_total : 0}</div>
                    </div>

                      <h5 className="allplaces-card-title">{items?.name}</h5>
